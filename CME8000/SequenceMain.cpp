...
BOOL CSequenceMain::Select_UnloadTrayPos(int &nTrayPosX, int &nTrayPosY)
{
	...
	int nTrayY = m_pEquipData->nUnloadTrayY;	// 6
	int nTrayX = m_pEquipData->nUnloadTrayX;	// 5
	for (int y = 0; y < nTrayY; y++) {
		for (int x = 0; x < nTrayX; x++) {
			if (gData.InfoUnloadTray[y][x] == 0) { nTrayPosX = x; nTrayPosY = y; return TRUE; }
		}
	}
	return FALSE;
}
			...
			g_objAJinAXL.Is_MoveDone(AX_UNLOAD_PICKER_X, dUpX))
		{
			...
			if (Select_UnloadPickerPos(nUpStart, nUpPickCnt)) {
				if (Select_UnloadTrayPos(nUpPosX, nUpPosY)){
					...
					nUpTrayCnt = m_pEquipData->nUnloadTrayX - nUpPosX;
...
BOOL CSequenceMain::UnloadStage1_Run()
{
	...
	case 1:		// 안전확인
		if (g_objCommon.Check_Position(AX_UNLOAD_STAGE1_Y, 0) && g_objCommon.Check_Position(AX_UNLOAD_STAGE1_Z, 1) &&
			g_objCommon.Get_UnloadTrayMasterSlaveOut(1) && !m_pDX05->iUnloadStage1Exist && !m_pDX05->iUnloadStage1Exist2 && !m_pDX05->iUnloadStage1Exist3 &&
